
import "./Group.css";
import MyChart from "./MyChart";
import MyPieChart from "./PieChart";




const Group = () => {
  

  return (
    <div className="analyzes3-c1-parent">
      <div className="analyzes3-c1">
        <div className="data-in">
          <div className="data-in-inner">
            <div className="rectangle-parent">
              <div className="group-child" />
              <div className="line-parent">
                <div className="frame-child" />
                <div className="frame-parent">
                  <div className="group-wrapper">
                    <img
                      className="frame-item"
                      alt=""
                      src="/group-625873@2x.png"
                    />
                  </div>
                  <div className="emissionrevenue">Category-1</div>
                </div>
                <div className="frame-group">
                  <div className="ellipse-wrapper">
                    <img
                      className="frame-inner"
                      alt=""
                      src="/ellipse-8@2x.png"
                    />
                  </div>
                  <img
                    className="fedrop-down-icon"
                    alt=""
                    src="/fedropdown@2x.png"
                  />
                  <div className="notification-1-wrapper">
                    <div className="notification-1">
                      <img
                        className="vector-icon"
                        alt=""
                        src="/vector@2x.png"
                      />
                      <div className="wrapper">
                        <div className="div">1</div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="frame-container">
                <div className="button-wrapper">
                  <div className="button">
                    <img
                      className="cropped-peters-surgical-logo-c-icon"
                      alt=""
                      src="/croppedpeters-surgical-logo-cmjn300x1093x-1@2x.png"
                    />
                  </div>
                </div>
                <div className="button-parent">
                  <div className="button1">
                    <div className="data-in-wrapper">
                      <div className="analyze">Data-in</div>
                    </div>
                  </div>
                  <div className="button2">
                    <div className="energy">Energy</div>
                    <img
                      className="fedrop-down-icon1"
                      alt=""
                      src="/fedropdown@2x.png"
                    />
                    <img
                      className="material-symbolsenergy-progra-icon"
                      alt=""
                      src="/materialsymbolsenergyprogramsavingoutline@2x.png"
                    />
                  </div>
                  <div className="button3">
                    <div className="mditruck-cargo-container" />
                    <div className="water-and-effluents">
                      Water and Effluents
                    </div>
                    <img
                      className="fedrop-down-icon2"
                      alt=""
                      src="/fedropdown@2x.png"
                    />
                    <img
                      className="mdiwater-outline-icon"
                      alt=""
                      src="/mdiwateroutline@2x.png"
                    />
                  </div>
                </div>
                <div className="button-group">
                  <div className="button4">
                    <div className="data-in-wrapper">
                      <div className="analyze">Analyze</div>
                      <div className="frame-div" />
                    </div>
                  </div>
                  <div className="button5">
                    <img
                      className="material-symbolsenergy-progra-icon1"
                      alt=""
                      src="/materialsymbolsenergyprogramsavingoutline@2x.png"
                    />
                    <div className="energy1">Energy</div>
                  </div>
                  <div className="button6">
                    <img
                      className="ridelete-bin-6-line-icon"
                      alt=""
                      src="/rideletebin6line@2x.png"
                    />
                    <div className="waste">Waste</div>
                    <div className="eta">(βeta)</div>
                  </div>
                </div>
                <div className="rectangle-group">
                  <div className="rectangle-div" />
                  <div className="open-help-centre">Open Help Centre</div>
                </div>
              </div>
              <div className="line-group">
                <div className="line-div" />
                <div className="powered-by-parent">
                  <div className="powered-by">Powered by</div>
                  <img
                    className="longsight-1-icon"
                    alt=""
                    src="/longsight-1@2x.png"
                  />
                </div>
              </div>
              <div className="group-container">
                <div className="frame-wrapper">
                  <div className="frame-parent1">
                    <div className="purchased-goods-and-services-parent">
                      <div className="purchased-goods-and">
                        Purchased goods and Services
                      </div>
                      <img
                        className="mingcuteinformation-line-icon"
                        alt=""
                        src="logo3.png"
                      />
                      <i className="contribution-to-scope3">
                        contribution to Scope3
                      </i>
                    </div>
                    <div className="frame-child1" />
                    <div className="frame-wrapper1">
                      <div className="frame-wrapper2">
                        <div className="co2e-parent">
                          <div className="co2e">441510.9 CO2e</div>
                          <div className="yoy-wrapper">
                            <div className="yoy">25%YOY↑</div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="frame-parent2">
                <div className="purchased-goods-and-services-parent">
                  <div className="purchased-goods-and">
                    <p className="purchased-good-and">
                      Purchased good and services
                    </p>
                    <p className="purchased-good-and">to revenue ratio</p>
                  </div>
                  <img
                    className="mingcuteinformation-line-icon"
                    alt=""
                    src="/mingcuteinformationline@2x.png"
                  />
                  <img className="vector-icon1" alt="" src="/logo3.png" />
                </div>
                <div className="frame-child1" />
                <div className="frame-wrapper3">
                  <div className="frame-wrapper4">
                    <div className="co2e-group">
                      <div className="co2e">323586.2 CO2e</div>
                      <div className="yoy-wrapper">
                        <div className="yoy1">24%YOY↓</div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="frame-parent3">
            <div className="group-parent">
              <div className="group-div">
                <div className="top-3-suppliers-contributing-t-parent">
                  <div className="top-3-suppliers">
                    Top 3 Suppliers contributing to Category-1
                  </div>
                  <div className="frame-wrapper5">
                    <div />
                  </div>
                  <div className="frame-parent4">
                    <div className="rectangle-container">
                      <div className="frame-child3" />
                      <div className="energy1">Supplier-1</div>
                      <div className="div1">64%</div>
                    </div>
                    <div className="rectangle-container">
                      <div className="frame-child4" />
                      <div className="energy1">Supplier-2</div>
                      <div className="div1">32%</div>
                    </div>
                    <div className="rectangle-container">
                      <div className="frame-child5" />
                      <div className="energy1">Supplier-3</div>
                      <div className="div3">
                        <span>15</span>
                        <span className="span">{` `}</span>
                        <span className="span1">%</span>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="frame-child6" />
            </div>
            <div className="frame-child7" />
            <div className="frame-parent5">
              <div className="emissionrevenue-parent">
                <div className="emissionrevenue">Emission/Revenue</div>
                <img className="vector-icon2" alt="" src="/vector@2x.png" />
              </div>
              <div className="frame-child8" />
              <div className="frame-child9" />
              <MyChart/>
              
            </div>
            <div className="group-parent1">
              <div className="frame-parent6">
                <div className="line-wrapper">
                  <div className="frame-child10" />
                </div>
                <div className="material-symbolsarrow-drop-do-parent">
                  <div className="material-symbolsarrow-drop-do">
                    <img className="vector-icon3" alt="" src="/vector@2x.png" />
                  </div>
                  <div className="frame-child11" />
                  <div className="download-wrapper">
                    <div className="emissionrevenue">Download</div>
                  </div>
                </div>
                <div className="solardocument-bold-wrapper">
                  <div className="solardocument-bold" />
                </div>
                <img className="vector-icon4" alt="" src="/logo3.png" />
              </div>
              <div className="supplier-wise">Supplier-wise</div>
              <div className="material-symbolsarrow-drop-do-group">
                <div className="material-symbolsarrow-drop-do">
                  <img className="vector-icon5" alt="" src="/logo4.png" />
                </div>
                <div className="frame-child11" />
                <div className="download-container">
                  <div className="emissionrevenue">Download</div>
                </div>
              </div>
            </div>
          </div>
          <div className="frame-parent7">
            <div className="frame-wrapper5">
              <div />
            </div>
            <div className="frame-parent8">
              <div />
              <div />
            </div>
            <div className="frame-parent9">
              <div className="emission-by-supplier-parent">
                <div className="emissionrevenue">Emission by Supplier</div>
                <div className="full-screen-parent">
                  <div className="full-screen">
                    <img className="vector-icon6" alt="" src="/logo1.png" />
                  </div>
                  <div className="more-horiz">
                    <img className="vector-icon7" alt="" src="/logo2.png" />
                  </div>
                </div>
              </div>
             <MyPieChart/>
            
            </div>
          </div>
          <div className="frame-parent10">
            <div className="total-number-of-reached-suppli-wrapper">
              <div className="total-number-of-container">
                <p className="purchased-good-and">Total number of</p>
                <p className="purchased-good-and">reached suppliers</p>
              </div>
            </div>
            <div className="container">
              <div className="div">143</div>
            </div>
          </div>
          <div className="frame-parent11">
            <div className="purchased-goods-and-services-parent">
              <div className="purchased-goods-and">
                Purchased goods and Services
              </div>
              <img
                className="mingcuteinformation-line-icon"
                alt=""
                src="/mingcuteinformationline@2x.png"
              />
              <i className="contribution-to-scope3">contribution to Scope3</i>
            </div>
            <div className="frame-child1" />
            <div className="frame-wrapper1">
              <div className="frame-wrapper2">
                <div className="co2e-parent">
                  <div className="co2e">441510.9 CO2e</div>
                  <div className="yoy-wrapper">
                    <div className="yoy">25%YOY↑</div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="frame-parent11">
            <div className="purchased-goods-and-services-parent">
              <div className="purchased-goods-and">Category-1</div>
              <img
                className="mingcuteinformation-line-icon"
                alt=""
                src="/mingcuteinformationline@2x.png"
              />
              <img className="vector-icon8" alt="" src="/logo3.png" />
              <i className="contribution-to-scope3">contribution to Scope3</i>
            </div>
            <div className="frame-child1" />
            <div className="frame-wrapper1">
              <div className="frame-wrapper10">
                <div className="of-scope3-parent">
                  <div className="of-scope3">32% of Scope3</div>
                  <div className="yoy-wrapper">
                    <div className="yoy">25%YOY↑</div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="frame-parent13">
            <div className="frame-parent14">
              <div className="line-container">
                <div className="frame-child15" />
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="emissionrevenue">Adani</div>
                </div>
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="emissionrevenue">2023</div>
                </div>
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="emissionrevenue">24</div>
                </div>
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="div7">
                    <span>25%</span>
                    <span className="span2">↑</span>
                  </div>
                </div>
                <div className="frame-child15" />
              </div>
              <div className="line-parent1">
                <div className="frame-child15" />
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="emissionrevenue">Ambani</div>
                </div>
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="emissionrevenue">2023</div>
                </div>
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="emissionrevenue">25</div>
                </div>
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="div10">
                    <span>27%</span>
                    <span className="span2">↓</span>
                  </div>
                </div>
                <div className="frame-child15" />
              </div>
              <div className="line-parent1">
                <div className="frame-child15" />
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="emissionrevenue">PwC</div>
                </div>
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="emissionrevenue">2023</div>
                </div>
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="emissionrevenue">32</div>
                </div>
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="div10">
                    <span>24%</span>
                    <span className="span2">↓</span>
                  </div>
                </div>
                <div className="frame-child15" />
              </div>
              <div className="line-parent1">
                <div className="frame-child15" />
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="emissionrevenue">Vijaya steels</div>
                </div>
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="emissionrevenue">2023</div>
                </div>
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="emissionrevenue">38</div>
                </div>
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="div10">
                    <span>28%</span>
                    <span className="span2">↓</span>
                  </div>
                </div>
                <div className="frame-child15" />
              </div>
              <div className="line-parent1">
                <div className="frame-child15" />
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="emissionrevenue">Tata</div>
                </div>
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="emissionrevenue">2023</div>
                </div>
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="emissionrevenue">37</div>
                </div>
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="div7">
                    <span>34%</span>
                    <span className="span2">↑</span>
                  </div>
                </div>
                <div className="frame-child15" />
              </div>
              <div className="line-parent1">
                <div className="frame-child15" />
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="emissionrevenue">Visaka Steel</div>
                </div>
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="emissionrevenue">2023</div>
                </div>
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="emissionrevenue">45</div>
                </div>
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="div7">
                    <span>67%</span>
                    <span className="span2">↑</span>
                  </div>
                </div>
                <div className="frame-child15" />
              </div>
              <div className="line-parent1">
                <div className="frame-child15" />
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="emissionrevenue">Alibaba</div>
                </div>
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="emissionrevenue">2023</div>
                </div>
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="emissionrevenue">58</div>
                </div>
                <div className="frame-child15" />
                <div className="adani-wrapper">
                  <div className="div10">
                    <span>54%</span>
                    <span className="span2">↓</span>
                  </div>
                </div>
                <div className="frame-child15" />
              </div>
            </div>
            <div className="group-child6" />
            <div className="solardocument-bold-container">
              <div className="solardocument-bold1" />
            </div>
          </div>
          <div className="line-parent7">
            <div className="frame-child57" />
            <div className="frame-child58" />
            <div className="frame-child59" />
            <div className="supplier-wrapper">
              <div className="supplier">Supplier</div>
            </div>
            <div className="year-wrapper">
              <div className="supplier">YEAR</div>
            </div>
            <div className="emissionrevenue-ratio-wrapper">
              <div className="supplier">EMISSION/REVENUE RATIO</div>
            </div>
            <div className="mom-wrapper">
              <div className="supplier">MOM</div>
            </div>
            <div className="momyear-wise">MOM(YEAR WISE)</div>
            <div className="frame-child60" />
          </div>
        </div>
      </div>
      <div className="frame-parent15">
        <div className="show-tline-mar23-apr23-parent">
          <div className="div">
            <span>{`Show Tline: `}</span>
            <span className="mar23-apr23">Mar’23 - Apr’23</span>
          </div>
          <div className="frame-child61" />
          <img className="vector-icon9" alt="" src="/vector@2x.png" />
        </div>
        <div className="material-symbolsarrow-drop-do2" />
      </div>
    </div>
  );
};

export default Group;
